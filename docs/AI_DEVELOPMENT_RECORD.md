...
5. **缺乏领域规范层面的主动性**——AI 按指令实现了单口令鉴权，但全程未对"任何管理员可见全部原始数据"这一权限模型提出异议或替代方案。多管理员分群体的数据可见性隔离，是开发者在试点结束后的隐私复盘中自行识别的，已列为下一版本首要改进。这说明 AI 能忠实执行给定的需求，但对所在领域的伦理规范（如心理测评中的数据最小可见原则）缺少主动审视。

**始终由人决策的事项**：构念与题面方向、反向题认定、计分规则、架构选型（SCF+TOS+Pages）、数据安全策略（测试隔离、写前快照、密钥只走环境变量）、数据可见性权限模型（识别单一口令的局限并规划多群体隔离）、以及每一段生成代码的审查与验收。
